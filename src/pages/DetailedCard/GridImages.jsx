    ...
    <div className=" w-full  h-3/6 rounded-2xl">
      <div className="grid grid-cols-3 h-full w-full grid-rows-2 rounded-3xl gap-3 ">
        <div className="row-span-2 rounded-lg ">
          <img
            className="object-cover w-full h-full rounded-lg hover:brightness-90 cursor-pointer"
            src={card.thumbnail}
            alt={card.title}
          ></img>
        </div>
        ...
        {additionalImages &&
          additionalImages.map((image, index) => (
            <div
              key={index}
              className={`rounded-lg hover:brightness-90 cursor-pointer ${index >= 2 ? `col-start-${index}` : ""}`}
            >
              <img
